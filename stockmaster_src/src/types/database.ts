...
export interface Store { id: string; company_id: string; name: string; address: string | null; is_active: boolean; created_at: string }
...
export interface Sale { id:string; company_id:string; store_id:string|null; customer_id:string|null; reference:string|null; subtotal:number; discount_total:number; total:number; cost_total:number; gross_profit:number; currency_code:string; secondary_currency_code:string|null; secondary_exchange_rate:number|null; exchange_rate_effective_at:string|null; payment_method:string|null; created_by:string|null; created_at:string; store?:{name:string}|null; creator?:{full_name:string}|null; sale_items?:SaleItem[] }
export interface SaleItem { id:string; sale_id:string; product_id:string; product_variant_id:string|null; purchase_price_snapshot:number; sale_price:number; quantity:number; discount:number; gross_profit:number; line_total:number; product?:{name:string;sku:string}|null; variant?:{name:string;sku:string}|null }
export interface SaleStockItem { stockLevelId:string; productId:string; variantId:string|null; name:string; sku:string; salePrice:number; purchasePrice:number; available:number; imageUrl:string|null }
