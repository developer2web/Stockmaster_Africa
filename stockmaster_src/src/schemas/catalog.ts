...
const nonNegative=z.string().trim().min(1,'Valeur requise').refine(v=>Number.isFinite(Number(v))&&Number(v)>=0,'Valeur invalide');
const optionalNumber=z.string().trim().refine(v=>v===''||(Number.isFinite(Number(v))&&Number(v)>=0),'Valeur invalide');
