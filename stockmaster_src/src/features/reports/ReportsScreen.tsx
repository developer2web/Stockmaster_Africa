...
import { exportFinancialExcel, exportFinancialPdf } from './export';
...
export default function ReportsScreen() {
  ...
  const cash = useQuery({ queryKey: ['report-cash-balance', company, storeId], queryFn: () => getCashBalance(company,storeId), enabled: !!company && canReadCash });
  const details = useQuery({ queryKey: ['report-financial-details', company, dates.start, dates.end, storeId], queryFn: () => getFinancialDetails(company,dates.start,dates.end,storeId), enabled: !!company && validDates });
  const data = report.data;
  const cashBalance = cash.data ?? 0;
  const periodLabel = `${dates.start} au ${dates.end}`;
  const runExport = async (format: 'pdf' | 'excel') => {
    if (!data) return;
    const feature = format === 'pdf' ? 'pdf_export' : 'excel_export';
    if (!canUseFeature(feature)) {
      setExportError(`L’export ${format.toUpperCase()} n’est pas inclus dans votre forfait.`);
      return;
    }
    setExportError('');
    setExporting(format);
    try {
      ...
      else await exportFinancialExcel(context);
    } catch (error) {
  ...
  return (
    <PermissionGuard permission={['daily_reports.read', 'monthly_reports.read']}>
      <AdminPage title="Bilan des activités">
        ...
        <SegmentedButtons value={view} onValueChange={(value) => setView(value as ReportView)} buttons={[{ value: 'global', label: 'Global', icon: 'view-dashboard-outline' }, { value: 'sales', label: 'Ventes', icon: 'cart-outline' }, { value: 'expenses', label: 'Dépenses', icon: 'cash-minus' }]} />
        {data && <Card mode="contained" style={{ backgroundColor: theme.colors.surface }}><Card.Content style={styles.exportRow}><View style={styles.grow}><Text variant="titleMedium" style={styles.bold}>Exporter le rapport détaillé</Text><Text style={{ color: theme.colors.onSurfaceVariant }}>PDF prêt à imprimer ou classeur Excel avec feuilles détaillées.</Text></View><AppButton mode="outlined" icon="file-pdf-box" loading={exporting === 'pdf'} disabled={!!exporting} onPress={() => void runExport('pdf')}>PDF</AppButton><AppButton mode="outlined" icon="microsoft-excel" loading={exporting === 'excel'} disabled={!!exporting} onPress={() => void runExport('excel')}>Excel</AppButton></Card.Content></Card>}
...
const styles = StyleSheet.create({
  ...
  list: { gap: 14 },
  rank: { gap: 5, minWidth: 260, flex: 1 },
