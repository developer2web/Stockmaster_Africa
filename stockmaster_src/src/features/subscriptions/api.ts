...
export async function getPaymentHistory(): Promise<PaymentTransaction[]> {
  const { data, error } = await supabase.from('payment_transactions')
    .select('id,plan_id,provider,provider_reference,billing_cycle,amount,currency,phone_number,status,failure_reason,confirmed_at,created_at,plan:plans(name)')
...
export async function checkPaymentStatus(transactionId: string) {
  ...
  if (error) throw new Error(error.message);
  if (data?.error) throw new Error(data.error);
  return data as { transactionId: string; status: PaymentTransaction['status']; subscriptionStatus?: string };
}
