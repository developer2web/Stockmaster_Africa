...
export async function getCompany(companyId: string) {
  const { data, error } = await supabase.from('companies').select('id,name,slug,country_code,country_name,default_currency_code,secondary_currency_code,currency_locked_at,created_at').eq('id', companyId).single(); fail(error); return data as {
