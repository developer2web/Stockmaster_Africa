    ...
    <View style={styles.screen}>
      <Card style={styles.card}>
        <Card.Content style={styles.content}>
          ...
          <Text variant="headlineSmall">{success ? 'Paiement confirmé' : terminal ? 'Paiement non confirmé' : 'Confirmation en cours'}</Text>
          <Text style={styles.center}>
            {success
              ? 'Votre forfait a été activé par le serveur.'
              : terminal
                ? `Statut reçu : ${status}. Aucun forfait n’a été activé.`
                : 'Validez la demande sur votre téléphone. Cette page ne peut pas activer le forfait elle-même.'}
          </Text>
          <AppButton
            onPress={async () => {
          ...
          >
            {success ? 'Ouvrir StockMaster' : 'Retour aux forfaits'}
          </AppButton>
        </Card.Content>
      </Card>
    </View>
