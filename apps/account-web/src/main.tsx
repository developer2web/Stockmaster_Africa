...
function App() {
  ...
  async function markNotifications() {
    ...
    try {
      ...
      setNotice(changed.size ? `${changed.size} notification${plural(changed.size)} marquée${plural(changed.size)} comme lue${plural(changed.size)}.` : 'Aucune nouvelle notification à marquer comme lue.');
      await load();
  ...
  return <>
    ...
    <div className="subscriptionLayout">
      ...
      <section className="panel planDetails">
        ...
        <div className="quotaGrid"><article><b>{subscription?.max_businesses ?? currentPlan?.max_businesses ?? 0}</b><span>Entreprise{plural(subscription?.max_businesses ?? currentPlan?.max_businesses ?? 0)}</span></article><article><b>{subscription?.max_stores ?? currentPlan?.max_stores ?? 0}</b><span>Boutique{plural(subscription?.max_stores ?? currentPlan?.max_stores ?? 0)} par entreprise</span></article><article><b>{subscription?.max_employees ?? currentPlan?.max_employees ?? 0}</b><span>Employé{plural(subscription?.max_employees ?? currentPlan?.max_employees ?? 0)} actif{plural(subscription?.max_employees ?? currentPlan?.max_employees ?? 0)} par entreprise</span></article></div>
      </section>
    </div>
