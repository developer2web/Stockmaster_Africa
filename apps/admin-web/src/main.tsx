...
function App() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      const missingSections=[e.error&&'erreurs',w.error&&'avertissements',em.error&&'suivi des emails'].filter(Boolean);
...
function WarningsPage({ data,emailSummary,run }: { data:PlatformWarning[];emailSummary:EmailDeliverySummary[];run:Run }) {
  ...
  const filtered=data.filter(item=>!(configurationMissing&&item.warning_type==='email_delivery')&&(!query.trim()||`${item.title} ${item.detail} ${item.company_names.join(' ')}`.toLowerCase().includes(query.toLowerCase().trim()))&&(severity==='all'||item.severity===severity)&&(state==='all'||item.status===state));
