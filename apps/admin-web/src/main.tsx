...
// Motif obligatoire (au moins 3 caractères) avant une action importante :
// renvoie le motif nettoyé, ou null si annulé/trop court (l'appelant doit
// alors abandonner l'action sans rien faire).
function promptReason(message: string): string | null {
  const reason = window.prompt(message)?.trim() ?? null;
...
async function toggleCompanyAccess(company: Company, run: Run) {
  ...
  if (company.is_active) {
    reason = promptReason(`Raison obligatoire de la suspension de ${company.name} :`);
    if (!reason) return;
      ...
      <div className="paymentDecision">
        ...
        <div className="modalActions paymentActionBar">
          ...
          {payment.archived_at ? <button className="successBtn" onClick={() => void manage('restore')}>Restaurer</button> : <>
            ...
            {canReview && <button className="warnBtn" onClick={() => void manage('reject')}>Refuser</button>}
...
function AccountDeletions({ data, run }: { data: DeletionRequest[]; run: Run }) {
  ...
  function updateStatus(request: DeletionRequest, nextStatus: string) {
    ...
    if (nextStatus === 'rejected') {
      note = promptReason(`Motif obligatoire pour refuser la demande de ${request.full_name || request.email} :`);
      if (!note) return;
    }
    void run(() => supabase.rpc('super_admin_update_account_deletion_request', { p_request_id: request.id, p_status: nextStatus, p_note: note }), 'Demande mise à jour.');
  }
  function anonymize(request: DeletionRequest) {
    const reason = promptReason(`Motif obligatoire pour anonymiser le compte de ${request.full_name || request.email} (nom/photo/email effacés, connexion bloquée, accès entreprise désactivés — les ventes et produits déjà créés restent conservés sans nom rattaché, action irréversible) :`);
    if (!reason) return;
    void run(() => supabase.rpc('super_admin_anonymize_account', { p_request_id: request.id, p_reason: reason }), 'Compte anonymisé.');
  }
  async function deletePermanently(request: DeletionRequest) {
    const reason = promptReason(`Motif obligatoire pour supprimer DÉFINITIVEMENT le compte de ${request.full_name || request.email} (refusé automatiquement s'il a déjà payé, ou si ça couperait la facturation d'une entreprise encore active pour d'autres personnes ; sinon aucune trace personnelle, action irréversible) :`);
    if (!reason) return;
    const ok = await run(() => supabase.rpc('super_admin_delete_account_permanently', { p_user_id: request.user_id, p_reason: reason }), 'Compte supprimé définitivement.');
    ...
    <Table heads={['Client', 'Email', 'Motif', 'Demandée le', 'Statut', 'Traitée par', 'Action']}>
      {filtered.map(request => <tr key={request.id}>
        ...
        <td><RowActions>
          ...
          {request.status !== 'rejected' && request.status !== 'completed' && <button className="warnBtn" onClick={() => updateStatus(request, 'rejected')}>Refuser</button>}
        </RowActions></td>
      </tr>)}
    </Table>
...
// Les actions d'une ligne restent visibles directement, sans clic sur un menu
// « ⋯ » caché au préalable : c'est plus rapide à voir et à utiliser.
...
function Table({ heads, children }: { heads: string[]; children: React.ReactNode }) {
  ...
  return <section className="dataSection">
    <div className="tableMeta">
      ...
      <small>{count ? `${(safePage - 1) * pageSize + 1}–${Math.min(safePage * pageSize, count)} affiché${plural(Math.min(safePage * pageSize, count) - ((safePage - 1) * pageSize + 1) + 1)}` : 'Données synchronisées avec Supabase'}</small>
    </div>
