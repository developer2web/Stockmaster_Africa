...
type View = 'Vue générale' | 'Entreprises' | 'Utilisateurs' | 'Abonnements' | 'Paiements' | 'Promotions' | 'Support' | 'Erreurs' | 'Avertissements' | 'Activité' | 'Paramètres';
...
const views: View[] = ['Vue générale', 'Entreprises', 'Utilisateurs', 'Abonnements', 'Paiements', 'Promotions', 'Support', 'Erreurs', 'Avertissements', 'Activité', 'Paramètres'];
const icons = ['⌂', '▦', '♧', '▤', '▣', '◇', '◉', '!', '⚠', '◷', '⚙'];
...
function TrialControls({ companies, run }: { companies: Company[]; run: Run }) { const { plans, planName } = usePlanCatalog(); const [companyId, setCompanyId] = useState(''); const [days, setDays] = useState(14); const [planCode, setPlanCode] = useState('basic'); const [defaultDays, setDefaultDays] = useState(14); const [trialEnabled, setTrialEnabled] = useState(false); const [rulesLoaded, setRulesLoaded] = useState(false); useEffect(() => { supabase.from('billing_settings').select('trial_days,trial_enabled').eq('id', true).single().then(({ data }) => { if (data) { setDefaultDays(data.trial_days); setDays(data.trial_days); setTrialEnabled(data.trial_enabled); setRulesLoaded(true); } }); }, []); function grant() { if (!companyId) return; const company = companies.find(item => item.id === companyId); const planLabel=planName(planCode); if (company && confirm(`Attribuer ${days} jours d’essai ${planLabel} à ${company.name} ?`)) void run(() => supabase.rpc('super_admin_grant_trial', { p_company_id: companyId, p_days: days, p_plan_code: planCode }), 'Essai gratuit attribué.'); } function saveDefaults() { if (!rulesLoaded) return; void run(() => supabase.from('billing_settings').update({ trial_days: defaultDays, trial_enabled: trialEnabled }).eq('id', true), 'Règles des nouveaux clients enregistrées.'); } return <section className="trialControls"><article><header><i>+</i><div><b>Attribuer un essai</b><span>Choisissez une entreprise, un forfait et une durée.</span></div></header><label>Entreprise<select value={companyId} onChange={event => setCompanyId(event.target.value)}><option value="">Sélectionner une entreprise</option>{companies.map(company => <option key={company.id} value={company.id}>{company.name}</option>)}</select></label><label>Forfait d’essai<select value={planCode} onChange={event => setPlanCode(event.target.value)}>{plans.map(plan => <option key={plan.code} value={plan.code}>{plan.name}</option>)}</select></label><label>Durée en jours<input type="number" min="1" max="90" value={days} onChange={event => setDays(Number(event.target.value))}/></label><button className="primary" disabled={!companyId || !plans.some(plan => plan.code === planCode) || days < 1 || days > 90} onClick={grant}>Attribuer l’essai</button></article><article><header><i>◷</i><div><b>Nouveaux clients</b><span>Le forfait choisi à l’inscription est activé pendant l’essai.</span></div></header><label className="compactSwitch">Essai automatique<input type="checkbox" checked={trialEnabled} disabled={!rulesLoaded} onChange={event => setTrialEnabled(event.target.checked)}/></label><label>Durée par défaut<input type="number" min="1" max="90" value={rulesLoaded ? defaultDays : ''} disabled={!rulesLoaded} onChange={event => setDefaultDays(Number(event.target.value))}/></label><button className="detailsBtn" disabled={!rulesLoaded} onClick={saveDefaults}>Enregistrer la règle</button>{!rulesLoaded&&<p>Règle non chargée. Actualisez pour la consulter.</p>}</article><article className="offerControl"><header><i>%</i><div><b>Offre commerciale</b><span>Transformez l’avantage en jours gratuits, montant ou pourcentage.</span></div></header><div className="offerTypes"><span>Jours gratuits</span><span>Montant fixe</span><span>Pourcentage</span></div><button className="detailsBtn" onClick={() => window.dispatchEvent(new Event('stockmaster:open-promotions'))}>Créer une promotion</button></article></section>; }
...
function ErrorsPage({ data, run }: { data: ErrorEvent[]; run: Run }) {
  ...
  return <><Title>Erreurs techniques</Title><div className="pageSummary"><article><i>!</i><div><span>Erreurs ouvertes</span><b>{open.length}</b></div></article><article><i>×</i><div><span>Fatales</span><b>{fatal.length}</b></div></article><article><i>24h</i><div><span>Dernières 24 heures</span><b>{recent.length}</b></div></article></div><div className="warningBox">Les erreurs internes sont enregistrées ici pour le Super Admin. Les utilisateurs voient uniquement un message clair, jamais la pile technique complète.</div><Toolbar search={query} setSearch={setQuery}><select value={severity} onChange={event=>setSeverity(event.target.value)}><option value="all">Toutes les gravités</option><option value="warning">Avertissements</option><option value="error">Erreurs</option><option value="fatal">Fatales</option></select><select value={state} onChange={event=>setState(event.target.value)}><option value="open">Ouvertes</option><option value="resolved">Résolues</option><option value="all">Toutes</option></select><select value={period} onChange={event=>setPeriod(event.target.value)}><option value="1">24 heures</option><option value="7">7 jours</option><option value="30">30 jours</option><option value="all">Toute la période</option></select>{(query||severity!=='all'||state!=='open'||period!=='30')&&<button className="resetFilters" onClick={()=>{setQuery('');setSeverity('all');setState('open');setPeriod('30')}}>Réinitialiser</button>}</Toolbar><Table heads={['Gravité','Code et message','Entreprise','Appareil','Date','Statut','Action']}>{filtered.map(item=><tr key={item.id}><td><span className={`incidentSeverity ${item.severity}`}>{item.severity}</span></td><td><div className="incidentMessage"><b>{item.code}</b><span>{item.message}</span><details><summary>Détails techniques</summary><pre>{JSON.stringify(item.context??{},null,2)}</pre></details></div></td><td>{item.company?.name??'Plateforme'}<small className="cellNote">{item.user?.full_name??'Utilisateur'}</small></td><td>{item.platform??'—'}<small className="cellNote">v{item.app_version??'—'}</small></td><td>{new Date(item.created_at).toLocaleString('fr-FR')}</td><td><Badge ok={Boolean(item.resolved_at)}>{item.resolved_at?'Résolue':'Ouverte'}</Badge>{item.resolution_note&&<small className="cellNote">{item.resolution_note}</small>}</td><td><ActionMenu><button className={item.resolved_at?'detailsBtn':'successBtn'} onClick={()=>update(item,!item.resolved_at)}>{item.resolved_at?'Rouvrir':'Résoudre'}</button></ActionMenu></td></tr>)}</Table>{!filtered.length&&<div className="monitoringEmpty"><i>✓</i><b>Aucune erreur correspondante</b><span>La plateforme ne présente aucun incident pour ces filtres.</span></div>}</>;
}
