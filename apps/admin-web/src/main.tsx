...
function App() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      const [s, c, u, p, pr, t, a, b, subscriptions, e, w, em] = await Promise.all([
...
function Subscriptions({ companies, stats, run }: { companies: Company[]; stats: Stats | null; run: Run }) {
  ...
  return <><Title>Essais et abonnements</Title>
    ...
    <Toolbar search={query} setSearch={setQuery}>{mode === 'subscriptions' && <select value={subscriptionStatus} onChange={event => setSubscriptionStatus(event.target.value)}><option value="all">Tous les statuts</option><option value="trialing">En essai</option><option value="active">Actifs</option><option value="past_due">Paiement en retard</option><option value="expired">Expirés</option><option value="none">Sans abonnement</option></select>}<select value={plan} onChange={event => setPlan(event.target.value)}><option value="all">Tous les forfaits</option><option value="basic">Basic</option><option value="pro">Pro</option><option value="premium">Business</option><option value="none">Sans forfait</option></select>{(query || subscriptionStatus !== 'all' || plan !== 'all') && <button className="resetFilters" onClick={() => { setQuery(''); setSubscriptionStatus('all'); setPlan('all'); }}>Réinitialiser</button>}</Toolbar>
    {mode === 'trials' ? <>{filtered.length ? <div className="trialGrid">{filtered.map(company => <TrialCard key={company.id} company={company} changePlan={changePlan}/>)}</div> : <div className="trialsEmpty"><i>✓</i><b>Aucun essai correspondant</b><span>Vous pouvez attribuer un nouvel essai avec le formulaire ci-dessus.</span></div>}</> : <Table heads={['Entreprise', 'Plan actuel', 'Statut', 'Début', 'Expiration', 'Jours restants', 'Modifier le forfait']}>{filtered.map(company => <tr key={company.id}><td><div className="entityCell"><span>{company.name.slice(0, 1).toUpperCase()}</span><div><b>{company.name}</b><small>{company.slug ?? 'Entreprise StockMaster'}</small></div></div></td><td><span className="planTag">{company.plan_code === 'premium' ? 'Business' : company.plan_code ?? 'Sans forfait'}</span></td><td><Badge ok={company.subscription_status === 'active' || company.subscription_status === 'trialing'}>{company.subscription_status === 'trialing' ? 'En essai' : company.subscription_status ?? 'Aucun'}</Badge></td><td>{company.subscription_starts_at ? day(company.subscription_starts_at) : '—'}</td><td><b className="expiryDate">{company.subscription_expires_at ? day(company.subscription_expires_at) : '—'}</b></td><td><span className={`daysLeft ${remainingDays(company.subscription_expires_at) <= 5 ? 'urgent' : ''}`}>{company.subscription_expires_at ? `${remainingDays(company.subscription_expires_at)} j` : '—'}</span></td><td><select className="actionSelect" defaultValue="" onChange={event => { changePlan(company, event.target.value); event.currentTarget.value = ''; }}><option value="" disabled>Choisir un forfait</option><option value="basic">Basic · 30 jours</option><option value="pro">Pro · 30 jours</option><option value="premium">Business · 30 jours</option></select></td></tr>)}</Table>}
  </>;
}
