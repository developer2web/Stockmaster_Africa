...
values
  ...
  ('91000000-0000-4000-8000-000000000003','billing-admin@test.local','authenticated','authenticated');

update profiles set is_super_admin=true where id='91000000-0000-4000-8000-000000000003';
...
reset role;
insert into promotions(name,code,promotion_type,value,starts_at,expires_at,audience,is_active,created_by)
values('Welcome five','WELCOME5','fixed_amount',5,now()-interval '1 day',now()+interval '30 days','all',true,'91000000-0000-4000-8000-000000000003');

set local role authenticated;
select set_config('request.jwt.claim.role','authenticated',true);
select set_config('request.jwt.claim.sub','91000000-0000-4000-8000-000000000002',true);
select is(
  (select discount_amount from subscription_quote('92000000-0000-4000-8000-000000000002',(select id from plans where code='basic'),'monthly','WELCOME5')),
  5::numeric,
  'a valid promotion discounts the quoted price'
);
select lives_ok(
  $$select submit_manual_subscription_payment(
    '92000000-0000-4000-8000-000000000002',(select id from plans where code='basic'),
    'monthly','OM-TEST-2026',null,'WELCOME5','96000000-0000-4000-8000-000000000001'
  )$$,
  'an owner can submit a manual Orange Money payment'
);
select is((select status from payment_transactions where provider_reference='OM-TEST-2026'),'processing','manual payment waits for review');
select is((select count(*)::integer from subscriptions where company_id='92000000-0000-4000-8000-000000000002' and status='active'),0,'proof submission never activates a subscription');

select set_config('request.jwt.claim.sub','91000000-0000-4000-8000-000000000003',true);
select lives_ok(
  $$select super_admin_review_manual_payment((select id from payment_transactions where provider_reference='OM-TEST-2026'),true,null)$$,
  'the Super Admin can approve a verified manual payment'
);
select is((select status from payment_transactions where provider_reference='OM-TEST-2026'),'succeeded','approved manual payment becomes paid');
