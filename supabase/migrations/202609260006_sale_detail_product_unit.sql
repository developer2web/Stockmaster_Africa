-- Retour testeur du 26/09 : « Coca cola » (paquet, 35 000 GNF) et « coca cola »
-- (pièce, 7 000 GNF) étaient indiscernables dans le catalogue, le panier et le reçu.
-- Le détail de vente (source du reçu) renvoie maintenant l'unité du produit et
-- has_homonym (un autre produit de la même boutique porte le même nom, casse et
-- espaces ignorés), pour afficher la distinction sur le reçu. Reste de la fonction
-- inchangé (202608210003).
begin;
...
returns jsonb language sql stable security definer set search_path=public as $$
  select jsonb_build_object(
    ...
    'sale_items',coalesce((select jsonb_agg(jsonb_build_object(
      ...
      'product',jsonb_build_object('name',p.name,'sku',p.sku,'unit',p.unit,
        'has_homonym',exists(select 1 from public.products o where o.store_id=p.store_id and o.id<>p.id
          and lower(trim(o.name))=lower(trim(p.name)))),
...
revoke all on function public.get_sale_detail_safe(uuid) from public, anon;
