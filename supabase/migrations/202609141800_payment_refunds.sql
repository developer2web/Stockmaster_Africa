...
-- Procédure de remboursement : un paiement confirmé (Stripe ou Orange Money
-- manuel) peut désormais être marqué remboursé, avec traçabilité complète
-- (qui, quand, pourquoi, référence du remboursement fournisseur le cas
-- échéant) et expiration automatique de l'abonnement que ce paiement précis
-- avait créé — jamais un abonnement plus récent, puisque chaque paiement
-- réussi crée sa propre ligne `subscriptions` (voir process_payment_webhook
-- et la branche 'confirm' ci-dessous). L'appel réel à l'API Stripe (requis
-- pour un remboursement Stripe) ne peut pas se faire depuis SQL : il est
-- fait par la nouvelle fonction Edge `refund-payment`, qui appelle ensuite
-- cette même RPC pour enregistrer le résultat — exactement comme
-- create-payment fait l'appel Stripe puis écrit en base.

alter table public.payment_transactions drop constraint if exists payment_transactions_status_check;
alter table public.payment_transactions
  add constraint payment_transactions_status_check
  check(status in ('pending','processing','succeeded','failed','cancelled','expired','refunded'));
...
create index if not exists payment_transactions_refunded_idx
  on public.payment_transactions(refunded_at, created_at desc);

-- Le nombre de paramètres change (ajout de p_refund_reference) : on supprime
-- explicitement l'ancienne signature à 3 arguments pour éviter toute
-- ambiguïté de surcharge côté PostgREST, plutôt que de compter sur un
-- paramètre par défaut ajouté en fin de liste.
drop function if exists public.super_admin_manage_payment(uuid,text,text);
...
begin
  ...
  if v_action='refund' then
    if v_payment.status='refunded' then return v_payment.id; end if;
    if v_payment.status<>'succeeded' then
      raise exception 'Seul un paiement confirmé peut être remboursé';
    end if;
    if length(v_reason)<3 then
      raise exception 'Le motif du remboursement est obligatoire';
    end if;
    -- Un remboursement Stripe exige un vrai appel à l'API Stripe (fait par
    -- la fonction Edge refund-payment, qui fournit alors p_refund_reference)
    -- avant que quoi que ce soit ne soit enregistré ici — impossible de
    -- « rembourser » un paiement Stripe directement depuis cette RPC.
    if v_payment.provider='stripe' and nullif(trim(coalesce(p_refund_reference,'')),'') is null then
      raise exception 'Le remboursement Stripe doit passer par la fonction refund-payment (appel API Stripe requis avant l''enregistrement).';
    end if;

    update public.payment_transactions
      set status='refunded',refunded_at=now(),refunded_by=auth.uid(),
          refund_reason=v_reason,refund_reference=nullif(trim(coalesce(p_refund_reference,'')),'')
      where id=v_payment.id;

    -- N'expire que l'abonnement que CE paiement a lui-même créé (lien 1:1
    -- via subscription_id), jamais « l'abonnement actif actuel » de
    -- l'entreprise — si un paiement plus récent a depuis pris le relais,
    -- il n'est pas touché.
    if v_payment.subscription_id is not null then
      update public.subscriptions
        set status='expired'
        where id=v_payment.subscription_id and status in ('trialing','active');
    end if;

    insert into public.payment_status_log(payment_transaction_id,old_status,new_status,source,payload)
      values(v_payment.id,'succeeded','refunded','super_admin_refund',
        jsonb_build_object('reason',v_reason,'reference',p_refund_reference,'provider',v_payment.provider));

    if v_payment.company_id is not null then
      insert into public.audit_logs(company_id,actor_id,action,entity_type,entity_id,payload,created_by)
      values(v_payment.company_id,auth.uid(),'refund_payment','payment_transactions',v_payment.id,
        jsonb_build_object('reason',v_reason,'reference',p_refund_reference,'amount',v_payment.amount,
          'currency',v_payment.currency,'provider',v_payment.provider),auth.uid());
    end if;
...
grant execute on function public.super_admin_manage_payment(uuid,text,text,text) to authenticated;
revoke all on function public.super_admin_manage_payment(uuid,text,text,text) from anon;

-- v3 du catalogue de paiements admin : ajoute les 3 colonnes de
-- remboursement. loadBillingPayments() (admin-web) tente v3, puis retombe
-- sur v2 puis la version historique, exactement comme le passage v1->v2
-- l'avait déjà établi.
create or replace function public.super_admin_billing_payments_v3()
returns table(
  ...
  refunded_at timestamptz,refund_reason text,refund_reference text
)
...
select pt.id,pt.company_id,c.name,au.email::text,p.name,pt.amount,pt.base_amount,
  ...
  pt.refunded_at,pt.refund_reason,pt.refund_reference
...
$$;
grant execute on function public.super_admin_billing_payments_v3() to authenticated;
revoke all on function public.super_admin_billing_payments_v3() from anon;
