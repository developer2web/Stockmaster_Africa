...
-- Jusqu'ici, les garde-fous et le nettoyage d'attribution ne vivaient que
-- dans super_admin_delete_account_permanently() : supprimer un utilisateur
-- directement dans le tableau de bord Supabase (Authentication > Users)
-- contournait tout ça et échouait pour la même raison qu'avant (clés
-- étrangères), ou pire, aurait pu réussir sans aucun garde-fou si les
-- données de ce compte avaient été nettoyées entre-temps.
--
-- Ce trigger déplace toute la logique (garde-fous + nettoyage) directement
-- sur auth.users : elle s'applique maintenant à TOUTE suppression, qu'elle
-- vienne du bouton Super Admin ou directement du tableau de bord Supabase.
...
drop trigger if exists protect_and_prepare_user_deletion on auth.users;
create trigger protect_and_prepare_user_deletion
before delete on auth.users
for each row execute function public.protect_and_prepare_user_deletion();

-- La fonction Super Admin s'appuie maintenant entièrement sur ce trigger :
-- plus besoin de dupliquer les garde-fous ni le nettoyage ici.
