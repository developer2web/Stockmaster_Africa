-- La vraie source du forfait d'un nouvel essai n'est pas create_business
-- (déjà corrigé dans 202609150042, mais sans effet réel) : c'est ce trigger
-- BEFORE INSERT sur subscriptions, qui écrase systématiquement plan_id pour
-- toute nouvelle ligne 'trialing' sans moyen de paiement — trouvé en direct
-- après qu'un essai fraîchement créé soit resté sur Basic malgré le
-- changement précédent (le diagnostic par lecture de migration ne l'avait
-- pas montré ; confirmé via information_schema.triggers).
-- Reprend exactement la version en production (vérifiée en direct avant
-- modification) : seul le repli par défaut change (basic -> pro) quand
-- l'utilisateur n'a pas explicitement choisi de forfait à l'inscription
-- (raw_user_meta_data->>'selected_plan' reste prioritaire dans tous les cas).
