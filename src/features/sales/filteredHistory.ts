...
export async function getFilteredSales(companyId: string, storeId: string, cursor: PageCursor, criteria: HistoryCriteria): Promise<Sale[]> {
  if (!criteria.search && !criteria.after && !criteria.before && !criteria.payment && criteria.status === 'all') {
    return getSales(companyId, storeId, cursor, false);
  }
  const cursorKey = cursor ? `${cursor.createdAt}:${cursor.id}` : 'first';
  return withOfflineCache(`filtered-sales:${companyId}:${storeId}:${cursorKey}:${JSON.stringify(criteria)}`, async () => {
