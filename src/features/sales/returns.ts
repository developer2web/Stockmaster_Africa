...
export async function recordSaleReturn(input:{saleId:string;items:{saleItemId:string;quantity:number;disposition:ReturnDisposition}[];refundMethod:string;note:string}, operationId = createOperationId()) {
  ...
  const {data,error}=await supabase.rpc('record_sale_return',{p_sale_id:input.saleId,p_items:input.items,p_refund_method:input.refundMethod,p_note:input.note.trim()||null,p_operation_id:operationId});
