...
import { Card, Dialog, FAB, HelperText, Portal, Switch, Text, TextInput } from 'react-native-paper';
...
export function ProductFormScreen({ id,initialBarcode,basePath='/products' }: { id?: string;initialBarcode?:string;basePath?:string }) {
  ...
  return <AdminPage title={id?'Fiche produit':'Nouveau produit'}>
    ...
    <View style={styles.form}><Card mode="outlined"><Card.Title title="Informations du produit" subtitle="Les champs marqués * sont obligatoires"/><Card.Content style={styles.formContent}><FormField control={control} name="name" label="Nom du produit" required autoFocus/><FormField control={control} name="description" label="Description (facultative)" multiline/><FormField control={control} name="barcode" label="Code-barres (facultatif)" keyboardType="numeric"/>
