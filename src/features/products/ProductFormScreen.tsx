...
export function ProductFormScreen({ id,initialBarcode,basePath='/products' }: { id?: string;initialBarcode?:string;basePath?:string }) {
  ...
  const { control, handleSubmit, reset,formState:{isValid,isDirty} } = useForm<ProductInput>({ resolver:zodResolver(productSchema), defaultValues:{...defaults,barcode:initialBarcode??''},mode:'onChange' });
  ...
  return <AdminPage title={id?'Fiche produit':'Nouveau produit'}>
    ...
    <View style={styles.form}><Card mode="outlined"><Card.Title title="Informations du produit" subtitle="Les champs marqués * sont obligatoires"/><Card.Content style={styles.formContent}><FormField control={control} name="name" label="Nom du produit" required autoFocus/><FormField control={control} name="description" label="Description (facultative)" multiline/><FormField control={control} name="sku" label="SKU" required autoCapitalize="characters"/><FormField control={control} name="barcode" label="Code-barres (facultatif)" keyboardType="numeric"/>
    ...
    <Controller control={control} name="unit" render={({field,fieldState})=><SelectField label="Unité" required value={field.value} options={[...unitOptions]} onChange={field.onChange} error={fieldState.error?.message}/>}/></Card.Content></Card><Card mode="outlined"><Card.Title title="Prix et stock" subtitle={`Montants en ${primaryCode}`}/><Card.Content style={styles.formContent}><FormField control={control} name="purchasePrice" label={`Prix d’achat (${primaryCode})`} required keyboardType="decimal-pad"/><FormField control={control} name="salePrice" label={`Prix de vente (${primaryCode})`} required keyboardType="decimal-pad"/>{!id&&<FormField control={control} name="initialQuantity" label="Quantité initiale (unités)" required keyboardType="decimal-pad"/>}<FormField control={control} name="lowStockThreshold" label="Seuil de stock faible (unités)" required keyboardType="decimal-pad"/>
