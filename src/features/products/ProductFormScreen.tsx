...
export function ProductFormScreen({ id,initialBarcode,basePath='/products',returnTo }: { id?: string;initialBarcode?:string;basePath?:string;returnTo?:string }) {
  ...
  return <AdminPage title={id?'Fiche produit':'Nouveau produit'}>
    ...
    <View style={styles.form}><Card mode="outlined"><Card.Title title="Informations du produit" subtitle="Les champs marqués * sont obligatoires"/><Card.Content style={styles.formContent}><FormField control={control} name="name" label="Nom du produit" required autoFocus/><FormField control={control} name="description" label="Description (facultative)" multiline/><ResponsiveFormGrid><FormField control={control} name="barcode" label="Code-barres (facultatif)" keyboardType="numeric"/><Controller control={control} name="categoryId" render={({field,fieldState})=><SelectField label="Catégorie" value={field.value} options={[{label:'Sans catégorie',value:null},...(categories.data??[]).filter(v=>v.is_active).map(v=>({label:v.name,value:v.id}))]} onChange={field.onChange} error={fieldState.error?.message}/>}/><Controller control={control} name="supplierId" render={({field,fieldState})=><SelectField label="Fournisseur" value={field.value} options={[{label:'Sans fournisseur',value:null},...(suppliers.data??[]).filter(v=>v.is_active).map(v=>({label:v.name,value:v.id}))]} onChange={field.onChange} error={fieldState.error?.message}/>}/><Controller control={control} name="unit" render={({field,fieldState})=><SelectField label="Unité" required value={field.value} options={[...unitOptions]} onChange={field.onChange} error={fieldState.error?.message}/>}/></ResponsiveFormGrid></Card.Content></Card><Card mode="outlined"><Card.Title title="Prix et stock" subtitle={`Montants en ${primaryCode}`}/><Card.Content style={styles.formContent}><ResponsiveFormGrid><FormField control={control} name="purchasePrice" label={`Prix d’achat (${primaryCode})`} required keyboardType="decimal-pad"/><FormField control={control} name="salePrice" label={`Prix de vente (${primaryCode})`} required keyboardType="decimal-pad"/>{!id&&<FormField control={control} name="initialQuantity" label="Quantité initiale (unités)" required keyboardType="decimal-pad"/>}<FormField control={control} name="lowStockThreshold" label="Seuil de stock faible (unités)" required keyboardType="decimal-pad"/></ResponsiveFormGrid>
    ...
    {!!save.error&&<HelperText type="error" visible>{readableError(save.error)}</HelperText>}<AppButton icon="content-save" loading={save.isPending} disabled={!company||!store||!isValid||!isDirty} onPress={handleSubmit(v=>save.mutate(v))}>Enregistrer</AppButton></Card.Content></Card></View>
