...
import { canAutofill, lotPriceFromUnit, unitPriceFromLot } from './bulkPricing';
...
// Retour testeur du 25/09 : la vente en gros est définitivement retirée du formulaire —
// règle de base rétablie, un produit est simple et vendu au détail par défaut. bulkEnabled
// reste à false pour toute nouvelle fiche (voir plus bas : les champs de lot restent dans le
// schéma uniquement pour ne pas effacer les données déjà enregistrées sur un produit existant
// qui en avait — ils ne sont plus jamais affichés ni modifiables ici).
...
export function ProductFormScreen({ id,initialBarcode,basePath='/products',returnTo }: { id?: string;initialBarcode?:string;basePath?:string;returnTo?:string }) {
  ...
  // bulkEnabled/bulkUnitLabel/bulkQuantity/bulkPrice/bulkPurchasePrice restent surveillés
  // uniquement pour préserver, au moment d'enregistrer, les données de lot déjà présentes
  // sur une fiche existante qui en avait avant le retrait de la vente en gros du formulaire
  // (voir le commentaire sur `defaults` plus haut) — plus aucun champ ne les affiche.
  ...
  // Le serveur refuse tout mouvement sur un produit archivé (« Produit invalide ») : pas de boutons dans ce cas (audit du 26/09).
  const archived=product.data?.is_active===false;
  ...
  return <AdminPage title={id?'Fiche produit':'Nouveau produit'} backTo={returnTo ? undefined : basePath} onBackPress={requestLeave}>
    ...
    {(!id || product.data) && <View style={styles.form}>
      ...
      {/* Retour testeur du 25/09 : la vente en gros est définitivement retirée du
          formulaire — règle de base rétablie, un produit est simple et vendu au détail. */}
      <Card mode="outlined">
        <Card.Title title="Prix" />
    ...
    {/* Audit du 26/09 : le bouton s'appelait « Supprimer » et annonçait un refus si le produit était utilisé,
        alors que le serveur archive toujours (is_active=false) et que le produit restait listé « Inactif ».
        Libellé et message alignés sur ce qui se passe réellement ; erreur affichée si l'archivage échoue. */}
    {id&&product.data?.is_active!==false&&<AppButton mode="outlined" destructive icon="archive-outline" onPress={()=>{remove.reset();setConfirm(true)}}>Archiver le produit</AppButton>}
    <ConfirmDialog visible={confirm} title="Archiver ce produit ?" message={remove.error?(/Archivage refusé/.test(remove.error.message)?'Archivage refusé : vous n’avez pas le droit de modifier les produits de cette boutique, ou l’abonnement n’est plus actif.':remove.error.message):'Le produit ne sera plus proposé à la vente et apparaîtra « Inactif » dans la liste. Ses ventes et son historique de stock sont conservés. Vous pourrez le réactiver avec l’option « Produit actif ».'} destructive confirmLabel="Archiver" loading={remove.isPending} onCancel={()=>setConfirm(false)} onConfirm={()=>remove.mutate()}/>
