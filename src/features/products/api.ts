...
import type { ProductInput, SupplierInput, VariantInput } from '@/schemas/catalog';
...
export async function getProduct(id: string): Promise<Product> {
  const { data, error } = await supabase.from('products')
    .select('id,company_id,store_id,supplier_id,name,description,sku,qr_code,barcode,unit,sale_price,low_stock_threshold,image_url,image_urls,is_active,created_at,bulk_unit_label,bulk_quantity,bulk_price,supplier:suppliers(name),product_variants(id,product_id,name,sku,barcode,attributes,sale_price,is_active)')
    .eq('id', id).single();
  fail(error);
