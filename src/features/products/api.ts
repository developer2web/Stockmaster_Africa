...
export async function saveProduct(companyId:string,storeId:string,v:ProductInput,id?:string):Promise<string>{const payload={company_id:companyId,store_id:storeId,name:v.name,description:empty(v.description),sku:(v.sku ?? '').trim() || '',barcode:empty(v.barcode),category_id:v.categoryId,supplier_id:v.supplierId,unit:v.unit,purchase_price:parseDecimal(v.purchasePrice),sale_price:parseDecimal(v.salePrice),low_stock_threshold:parseDecimal(v.lowStockThreshold),is_active:v.isActive};if(id){const{error}=await supabase.from('products').update(payload).eq('id',id);fail(error);return id}const{data,error}=await supabase.rpc('create_product_with_initial_stock',{p_store_id:storeId,p_name:v.name,p_description:v.description,p_sku:(v.sku ?? '').trim() || '',p_barcode:v.barcode,p_category_id:v.categoryId,p_supplier_id:v.supplierId,p_unit:v.unit,p_purchase_price:parseDecimal(v.purchasePrice),p_sale_price:parseDecimal(v.salePrice),p_low_stock_threshold:parseDecimal(v.lowStockThreshold),p_is_active:v.isActive,p_initial_quantity:parseDecimal(v.initialQuantity),p_operation_id:createOperationId()});fail(error);if(!data)throw new Error('Le produit n’a pas été créé.');return data as string}
