...
type ProductRow={Nom?:unknown;SKU?:unknown;Code_barres?:unknown;Unite?:unknown;Prix_achat?:unknown;Prix_vente?:unknown;Stock_initial?:unknown;Seuil_stock_faible?:unknown};
const units=new Set(['piece','carton','kg','litre','sac','paquet']);
export type ImportPreview={row:number;name:string;sku:string;valid:boolean;error?:string;data?:ProductRow};
...
export async function selectProductWorkbook():Promise<ImportPreview[]|null>{
  ...
  const seen=new Set<string>();return rows.map((data,index)=>{const name=String(data.Nom??'').trim();const sku=String(data.SKU??'').trim();const normalizedSku=sku.toLowerCase();const duplicate=seen.has(normalizedSku);if(normalizedSku)seen.add(normalizedSku);const unit=String(data.Unite??'piece').trim().toLowerCase();const numbers=[data.Prix_achat,data.Prix_vente,data.Stock_initial,data.Seuil_stock_faible].map(Number);const error=name.length<2?'Nom manquant':sku.length<2?'SKU manquant':duplicate?'SKU dupliqué dans le fichier':!units.has(unit)?'Unité invalide':numbers.some(value=>!Number.isFinite(value)||value<0)?'Prix, stock ou seuil invalide':undefined;return{row:index+2,name,sku,valid:!error,error,data}});
}

export async function importProductRows(companyId:string,storeId:string,rows:ImportPreview[]){const errors:string[]=[];let imported=0;for(const row of rows.filter(item=>item.valid&&item.data)){const data=row.data!;try{await saveProduct(companyId,storeId,{name:row.name,description:'',sku:row.sku,barcode:String(data.Code_barres??'').trim(),categoryId:null,supplierId:null,unit:String(data.Unite??'piece').trim().toLowerCase() as 'piece',purchasePrice:String(data.Prix_achat),salePrice:String(data.Prix_vente),initialQuantity:String(data.Stock_initial),lowStockThreshold:String(data.Seuil_stock_faible),isActive:true});imported+=1}catch(error){errors.push(`Ligne ${row.row}: ${error instanceof Error?error.message:'Erreur'}`)}}return{imported,errors}}
