...
export async function shareSalesExport(sales: Sale[]){const rows=sales.map(sale=>({Référence:sale.reference??'Vente',Date:new Date(sale.created_at).toLocaleString('fr-FR'),Client:sale.customer?.name??'',Paiement:sale.payment_method??'',Statut:sale.payment_status,Total:Number(sale.total),Montant_payé:Number(sale.amount_paid),Reste:Number(sale.amount_due)}));const book=XLSX.utils.book_new();XLSX.utils.book_append_sheet(book,XLSX.utils.json_to_sheet(rows),'Ventes');await shareBook(book,`StockMaster_Ventes_${new Date().toISOString().slice(0,10)}.xlsx`,'Exporter les ventes Excel')}
