...
export const productSchema=z.object({name:z.string().trim().min(2,'Nom requis').max(120),description:z.string().trim().max(250).default(''),sku:z.string().trim().max(80).default('SKU-AUTO'),barcode:z.string().trim().max(80).default(''),categoryId:z.string().uuid().nullable(),supplierId:z.string().uuid().nullable(),unit:z.enum(['piece','carton','kg','litre','sac','paquet']),purchasePrice:nonNegative,salePrice:nonNegative,initialQuantity:nonNegative,lowStockThreshold:nonNegative,isActive:z.boolean()});
...
export type CategoryInput=z.input<typeof categorySchema>; export type SupplierInput=z.input<typeof supplierSchema>; export type ProductInput=z.input<typeof productSchema>; export type VariantInput=z.input<typeof variantSchema>;
