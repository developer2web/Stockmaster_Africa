...
export function RequestAccessRemovalCard({ companyId }: { companyId: string }) {
  ...
  return <>
    <ListSection title="Quitter l’entreprise">
      <ListRow
        icon="account-arrow-left-outline"
        title="Demander le retrait de mon accès"
    ...
    <Portal>
      <Dialog style={styles.dialog} visible={open} onDismiss={() => !request.isPending && setOpen(false)}>
        <Dialog.Title>Demander le retrait de mon accès</Dialog.Title>
        <Dialog.Content style={{ gap: 10 }}>
          <Text>L’administrateur de l’entreprise recevra une notification et pourra retirer votre accès à cette entreprise. Cela ne supprime pas votre compte StockMaster.</Text>
        ...
        <Dialog.Actions style={{ flexWrap: 'wrap' }}>
          ...
          <AppButton loading={request.isPending} disabled={request.isPending} onPress={() => request.mutate()}>Envoyer la demande</AppButton>
        </Dialog.Actions>
      </Dialog>
    </Portal>
