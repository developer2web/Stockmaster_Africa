...
export function PlatformPage({ title, back = true, action, children }: PropsWithChildren<{ title: string; back?: boolean; action?: ReactNode }>) {
  ...
  const navigation=[
    ...
    {label:'Opérations',icon:'heart-pulse',route:'/(super-admin)/operations'},
  ] as const;
  ...
  return (
    <View style={[styles.screen, { backgroundColor: theme.colors.background }]}>
      <Appbar.Header elevated style={{ backgroundColor: theme.colors.surface }}>
        ...
        <Appbar.Content title={title} subtitle={compact ? undefined : 'Administration StockMaster'} titleStyle={compact ? styles.compactTitle : undefined} />
        {action}
      </Appbar.Header>
      <ScrollView contentContainerStyle={[styles.page, compact && styles.compactPage,compact&&{paddingBottom:92+insets.bottom}]} showsVerticalScrollIndicator={false}>{children}</ScrollView>
