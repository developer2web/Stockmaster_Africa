...
export default function CompanyScreen() {
  ...
  const settingsMutation=useMutation({mutationFn:()=>updateBusinessSettings(companyId,{...settings,taxRate:Number(settings.taxRate)||0,maxDiscountPercent:Math.min(100,Math.max(0,Number(settings.maxDiscountPercent)||0)),cashVarianceReasonThreshold:Math.max(0,Number(settings.cashVarianceReasonThreshold)||0),expenseApprovalThreshold:settings.expenseApprovalThreshold.trim()?Math.max(0,Number(settings.expenseApprovalThreshold)||0):null}),onSuccess:refresh});
