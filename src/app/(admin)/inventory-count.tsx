...
import { Card,Chip,HelperText,Searchbar,TextInput } from 'react-native-paper';
...
export default function InventoryCountScreen(){
  ...
  const rows=useMemo(()=>{const needle=search.trim().toLowerCase();return(inventory.data?.inventory_items??[]).filter(item=>(!needle||`${item.product?.name} ${item.product?.sku} ${item.variant?.name??''}`.toLowerCase().includes(needle))&&(!productId||item.product_id===productId))},[inventory.data,productId,search]);
  ...
  return <AdminPage title="Comptage physique" action={<AppButton mode="outlined" icon="barcode-scan" disabled={!id} onPress={()=>router.push({pathname:'/scanner' as never,params:{mode:'inventory',inventoryId:id}})}>Scanner</AppButton>}>
    ...
    <Searchbar placeholder="Produit ou SKU" value={search} onChangeText={setSearch}/>
