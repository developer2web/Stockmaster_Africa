...
export default function NotificationsScreen() {
  ...
  const usefulTypes = (type: string) => type === 'stock_out' || type === 'low_stock' || type === 'customer_debt' || type === 'cash_unclosed' || type === 'order_pending';
  const usefulPersistent = (persistent.data ?? []).filter((item) => usefulTypes(item.type));
  const usefulInternal = (query.data ?? []).filter((item) => usefulTypes(item.type));
  const unread = usefulPersistent.filter((item) => !item.read_at);
  ...
  return <AdminPage title="Notifications" action={unread.length ? <View style={{flexDirection:'row',flexWrap:'wrap',alignItems:'center',gap:8}}><Chip icon="bell-badge-outline">{unread.length} non lue{unread.length>1?'s':''}</Chip><AppButton compact mode="text" loading={mark.isPending} onPress={() => mark.mutate()}>Tout marquer comme lu</AppButton></View> : undefined}>
    ...
    {usefulPersistent.map((item) => <Card key={item.id} mode={item.read_at ? 'outlined' : 'contained'} style={{ backgroundColor: item.read_at ? undefined : theme.colors.primaryContainer }}><Card.Content style={{ flexDirection: 'row', alignItems: 'center', gap: 12 }}><Icon source={icon(item.type)} size={28} color={theme.colors.primary}/><Text style={{ flex: 1 }}><Text variant="titleMedium">{item.title}</Text>{'\n'}{item.body}{'\n'}<Text variant="labelSmall">{formatDateTime(item.created_at)}</Text></Text>{!item.read_at && <Chip>Nouveau</Chip>}</Card.Content></Card>)}
    {usefulInternal.map((item) => <Card key={`live-${item.id}`} mode="outlined"><Card.Content style={{ flexDirection: 'row', alignItems: 'center', gap: 12 }}><Icon source={icon(item.type)} size={28} color={item.severity === '3' ? '#C92A2A' : item.severity === '2' ? '#E67700' : '#1971C2'}/><Text style={{ flex: 1 }}><Text variant="titleMedium">{item.title}</Text>{'\n'}{item.body}</Text><Chip>{item.severity === '3' ? 'Urgent' : 'À voir'}</Chip></Card.Content></Card>)}
    {!query.isLoading && !persistent.isLoading && !usefulInternal.length && !usefulPersistent.length && <EmptyState icon="bell-check-outline" title="Tout va bien" message="Aucune alerte importante pour cette boutique."/>}
  </AdminPage>;
}
