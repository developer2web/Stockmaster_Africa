...
import { type ImportPreview,importProductRows,selectProductWorkbook } from '@/features/products/excel';
...
export default function ProductImport(){
  ...
  const pick=async()=>{setActionError('');try{setRows(await selectProductWorkbook(membership?.companyId??'',membership?.storeId??'')??[])}catch(error){setActionError(error instanceof Error?error.message:'Fichier illisible')}};
  ...
  return <FeatureGate feature="excel_export" label="Import Excel"><AdminPage title="Importer des produits"><Card mode="contained"><Card.Title title="Fichier Excel StockMaster" subtitle="Vérification avant toute création"/><Card.Content><Text>Préparez un fichier Excel avec les colonnes ci-dessous dans sa première feuille, puis sélectionnez-le. Les lignes invalides ne seront pas importées.</Text><Text style={{ marginTop: 12 }}>Colonnes : Nom, Code_barres, Unite, Prix_achat, Prix_vente, Stock_initial, Seuil_stock_faible.</Text><Text style={{ marginTop: 8 }}>Unités acceptées : piece, carton, kg, litre, sac, paquet. Maximum : 1 000 produits et 5 Mo.</Text></Card.Content><Card.Actions><AppButton icon="file-excel" onPress={()=>void pick()}>Choisir le fichier</AppButton></Card.Actions></Card>{rows.length>0&&<Chip>{valid}/{rows.length} lignes valides</Chip>}{rows.map(row=><Card key={row.row} mode="outlined"><Card.Title title={`Ligne ${row.row} • ${row.name||'Sans nom'}`} subtitle={row.valid?'Prêt à importer':row.error} right={()=> <Chip style={{marginRight:12}} icon={row.valid?'check':'alert'}>{row.valid?'Valide':'Erreur'}</Chip>}/></Card>)}{!!actionError&&<HelperText type="error" visible>{actionError}</HelperText>}{!!run.error&&<HelperText type="error" visible>{run.error.message}</HelperText>}{run.data?.errors.map(error=><HelperText key={error} type="error" visible>{error}</HelperText>)}<AppButton icon="database-import" loading={run.isPending} disabled={!valid||run.isPending} onPress={()=>run.mutate()}>Importer {valid} produit{plural(valid)}</AppButton></AdminPage></FeatureGate>
}
