...
import { Card, Checkbox, Dialog, FAB, HelperText, Portal, Switch, Text } from 'react-native-paper';
...
export default function EmployeesScreen() {
  ...
  const invite = useMutation({mutationFn:(values:EmployeeInput)=>inviteEmployee(values,companyId),onSuccess:async(data,values)=>{await qc.invalidateQueries({queryKey:['employees',companyId]});setInviteOpen(false);setCredentials({email:values.email.trim().toLowerCase(),password:data.temporaryPassword})}});
  ...
  const remove = useMutation({mutationFn:()=>deleteEmployee(editing!.id),onSuccess:async()=>{await qc.invalidateQueries({queryKey:['employees',companyId]});setEditing(null)}});
  ...
  return <AdminPage title="Employés" action={<FAB size="small" icon="account-plus" onPress={openInvite}/> }>
    <Card mode="outlined"><Card.Content><Text variant="titleMedium">Accès employé sans email</Text><Text>Un mot de passe est généré à l’ajout. Notez-le et transmettez-le directement à l’employé : il ne sera pas envoyé par email ni conservé sur cet écran.</Text></Card.Content></Card>
    ...
    {employees.data?.length?employees.data.map(e=><Card key={e.id} onPress={()=>openEmployee(e)}><Card.Title title={e.fullName} subtitle={`${e.roleName} • ${e.allStores?'Toutes les boutiques':e.storeNames.join(', ')||'Aucune boutique'}`} right={()=><Text style={{marginRight:16}}>{e.isActive?'Actif':'Suspendu'}</Text>}/></Card>):<EmptyState icon="account-multiple-plus" title="Aucun employé" message="Invitez votre équipe et attribuez précisément ses accès."/>}
    <Portal><Dialog visible={inviteOpen} onDismiss={()=>setInviteOpen(false)}><Dialog.Title>Ajouter un employé</Dialog.Title><Dialog.ScrollArea style={{paddingHorizontal:0}}><ScrollView contentContainerStyle={{gap:12,paddingHorizontal:24,paddingBottom:12}} keyboardShouldPersistTaps="handled"><FormField control={control} name="fullName" label="Nom complet"/><FormField control={control} name="email" label="Email" keyboardType="email-address" autoCapitalize="none"/><Controller control={control} name="roleId" render={({field,fieldState})=><SelectField label="Rôle" value={field.value} options={roleOptions} onChange={v=>field.onChange(v??'')} error={fieldState.error?.message}/>}/><Controller control={control} name="allStores" render={({field})=><Checkbox.Item label="Accès à toutes les boutiques" status={field.value?'checked':'unchecked'} onPress={()=>field.onChange(!field.value)}/>}/><Controller control={control} name="storeIds" render={({field,fieldState})=><Card mode="outlined"><Card.Title title="Boutiques autorisées"/><Card.Content>{(stores.data??[]).filter(s=>s.is_active).map(store=><Checkbox.Item key={store.id} label={store.name} status={field.value.includes(store.id)?'checked':'unchecked'} onPress={()=>field.onChange(field.value.includes(store.id)?field.value.filter(id=>id!==store.id):[...field.value,store.id])}/>) }{!!fieldState.error&&<HelperText type="error" visible>{fieldState.error.message}</HelperText>}</Card.Content></Card>}/>{!!invite.error&&<HelperText type="error" visible>{invite.error.message}</HelperText>}</ScrollView></Dialog.ScrollArea><Dialog.Actions><AppButton mode="text" onPress={()=>setInviteOpen(false)}>Annuler</AppButton><AppButton disabled={!roleOptions.length} loading={invite.isPending} onPress={handleSubmit(v=>invite.mutate(v))}>Ajouter</AppButton></Dialog.Actions></Dialog></Portal>
    <Portal><Dialog visible={!!editing} onDismiss={()=>setEditing(null)}><Dialog.Title>Accès de {editing?.fullName}</Dialog.Title><Dialog.ScrollArea style={{paddingHorizontal:0}}><ScrollView contentContainerStyle={{gap:12,paddingHorizontal:24,paddingBottom:12}} keyboardShouldPersistTaps="handled"><SelectField label="Rôle" value={editRole} options={roleOptions} onChange={v=>setEditRole(v??'')}/><Checkbox.Item label="Toutes les boutiques" status={editAllStores?'checked':'unchecked'} onPress={()=>setEditAllStores(value=>!value)}/>{!editAllStores&&(stores.data??[]).filter(s=>s.is_active).map(store=><Checkbox.Item key={store.id} label={store.name} status={editStores.includes(store.id)?'checked':'unchecked'} onPress={()=>setEditStores(current=>current.includes(store.id)?current.filter(id=>id!==store.id):[...current,store.id])}/>) }<Card mode="outlined"><Card.Title title="Compte actif" right={()=><Switch value={editActive} onValueChange={setEditActive} style={{marginRight:12}}/>}/></Card>{!!update.error&&<HelperText type="error" visible>{update.error.message}</HelperText>}{!!remove.error&&<HelperText type="error" visible>{remove.error.message}</HelperText>}</ScrollView></Dialog.ScrollArea><Dialog.Actions><AppButton mode="text" textColor="#C92A2A" loading={remove.isPending} onPress={()=>remove.mutate()}>Supprimer</AppButton><AppButton mode="text" onPress={()=>setEditing(null)}>Annuler</AppButton><AppButton disabled={!editRole||(!editAllStores&&!editStores.length)} loading={update.isPending} onPress={()=>update.mutate({roleId:editRole,storeIds:editStores,allStores:editAllStores,active:editActive})}>Enregistrer</AppButton></Dialog.Actions></Dialog></Portal>
    <Portal><Dialog visible={!!credentials} dismissable={false}><Dialog.Title>Identifiants de connexion</Dialog.Title><Dialog.Content style={{gap:14}}><Text>Copiez ces informations maintenant. Le mot de passe ne sera plus affiché après fermeture.</Text><Card mode="outlined"><Card.Content style={{gap:8}}><Text variant="labelLarge">Email</Text><Text selectable>{credentials?.email}</Text><Text variant="labelLarge">Mot de passe</Text><Text selectable variant="titleLarge">{credentials?.password}</Text></Card.Content></Card><HelperText type="info" visible>L’employé pourra le modifier dans Paramètres &gt; Mot de passe après avoir confirmé l’ancien.</HelperText></Dialog.Content><Dialog.Actions><AppButton onPress={()=>setCredentials(null)}>J’ai noté les identifiants</AppButton></Dialog.Actions></Dialog></Portal>
  </AdminPage>;
}
