...
export default function CashScreen() {
  ...
  return (
    <AdminPage title="Caisse">
      ...
      {!!closures.data?.length&&<><Text variant="titleLarge" style={styles.bold}>Dernières clôtures</Text>{closures.data.slice(0,7).map(item=><Card key={item.id} mode="outlined"><Card.Title title={new Date(`${item.closure_date}T12:00:00`).toLocaleDateString('fr-FR')} subtitle={`Attendu ${money(Number(item.expected_amount))} • Compté ${money(Number(item.counted_amount))}`} right={()=><Text style={{marginRight:16,color:Number(item.difference)===0?theme.colors.primary:theme.colors.error,fontWeight:'800'}}>{Number(item.difference)>0?'+':''}{money(Number(item.difference))}</Text>}/><Card.Content><Text style={styles.bold}>Clôture effectuée par : {item.closed_by_label}</Text>{item.note&&<Text>{item.note}</Text>}</Card.Content></Card>)}</>}
      ...
      {rows.map((item) => (
        <Card key={item.id} mode="contained" style={{ backgroundColor: theme.colors.surface }}>
          ...
          <Card.Actions>{(()=>{const data={title:item.transaction_type==='deposit'?'Reçu d’entrée de caisse':'Reçu de sortie de caisse',party:item.designation,partyLabel:'Opération',amount:Number(item.amount),balanceBefore:0,balanceAfter:0,date:item.created_at,reference:`CAISSE-${item.id.slice(0,8).toUpperCase()}`,company:membership?.companyName,store:item.store?.name??membership?.storeName,issuedBy:item.creator?.full_name||(membership?.role==='company_admin'?'Administrateur':'Employé'),amountLabel:item.transaction_type==='deposit'?'Montant encaissé':'Montant décaissé',showBalances:false};const printKey=`print-${item.id}`,shareKey=`share-${item.id}`;return <><AppButton mode="text" icon="printer" loading={receiptAction.runningKey===printKey} disabled={!!receiptAction.runningKey} onPress={()=>void receiptAction.run(printKey,()=>printPaymentReceipt(data,value=>formatForCurrency(value,item.currency_code)))}>Imprimer</AppButton><AppButton mode="text" icon="share-variant" loading={receiptAction.runningKey===shareKey} disabled={!!receiptAction.runningKey} onPress={()=>void receiptAction.run(shareKey,()=>sharePaymentReceipt(data,value=>formatForCurrency(value,item.currency_code)))}>Partager</AppButton></>})()}</Card.Actions>
        </Card>
      ))}
