...
import { wholeOrNaN } from '@/utils/number';
import { WholeNumberInput } from '@/components/forms/WholeNumberInput';
import { ValidatedInput } from '@/components/forms/ValidatedInput';
import { MAX_CASH_AMOUNT } from '@/constants/limits';
...
export default function CashScreen() {
  ...
  const mutation = useMutation({
    mutationFn: (options?: { confirmNegative?: boolean }) => createCashTransaction({ companyId, storeId, type: type!, designation, amount: wholeOrNaN(amount,{max:MAX_CASH_AMOUNT}), confirmNegative: options?.confirmNegative }),
  ...
  const wouldGoNegative = type === 'withdrawal' && wholeOrNaN(amount,{max:MAX_CASH_AMOUNT}) > 0 && balance - wholeOrNaN(amount,{max:MAX_CASH_AMOUNT}) < 0;
  const valid = !!storeId && designation.trim().length >= 2 && wholeOrNaN(amount,{max:MAX_CASH_AMOUNT}) > 0 && (!wouldGoNegative || canOverrideNegative);
  const closure=useMutation({mutationFn:()=>closeCash(storeId,wholeOrNaN(countedAmount,{max:MAX_CASH_AMOUNT}),closureNote),onSuccess:async()=>{await Promise.all([cache.invalidateQueries({queryKey:['cash-closures',companyId,storeId]}),cache.invalidateQueries({queryKey:['cash-session',companyId,storeId]})]);setClosureOpen(false);setCountedAmount('');setClosureNote('');setSuccessMessage('Caisse clôturée');}});
  // Demande explicite du propriétaire (17/09) : avec un seuil de contrôle à
  // 0, un écart de -1 GNF exigeait déjà une justification côté serveur
  // (close_store_cash), mais rien ne le signalait avant l'envoi — la note
  // restait visuellement "facultative" et le bouton actif quel que soit
  // l'écart, jusqu'au refus serveur.
  const companySettings = useQuery({ queryKey: ['company', companyId], queryFn: () => getCompany(companyId), enabled: !!companyId });
  const varianceThreshold = Number(companySettings.data?.cash_variance_reason_threshold ?? 0);
  const closureDifference = (wholeOrNaN(countedAmount,{max:MAX_CASH_AMOUNT}) || 0) - balance;
  const closureNoteRequired = Math.abs(closureDifference) > varianceThreshold;
  const opening=useMutation({mutationFn:()=>openCash(storeId,wholeOrNaN(openingAmount,{max:MAX_CASH_AMOUNT}),openingNote),onSuccess:async()=>{await cache.invalidateQueries({queryKey:['cash-session',companyId,storeId]});setOpeningOpen(false);setOpeningAmount('');setOpeningNote('');setSuccessMessage('Caisse ouverte');}});
  const requiresOpening=!!sessionStatus.data?.requiresOpening;
  const parsedOpeningAmount = wholeOrNaN(openingAmount,{max:MAX_CASH_AMOUNT});
  ...
  return (
    <AdminPage title="Caisse" description="">
        ...
        <Dialog visible={openingOpen && requiresOpening && canOpen} dismissable={!opening.isPending} onDismiss={() => !opening.isPending && setOpeningOpen(false)} style={[styles.openingDialog, { width: Math.min(460, width - 32) }]}>
          ...
          <Dialog.ScrollArea>
            <ScrollView style={{ maxHeight: Math.max(80, height * 0.45) }} contentContainerStyle={styles.dialogScroll} keyboardShouldPersistTaps="handled">
              ...
              <WholeNumberInput dense required label="Montant reçu" value={openingAmount} onChangeText={setOpeningAmount} max={MAX_CASH_AMOUNT} />
              <TextInput mode="outlined" dense label="Note en cas d’écart (facultatif)" accessibilityLabel="Note en cas d’écart (facultatif)" value={openingNote} onChangeText={setOpeningNote} multiline />
      ...
      <Portal><Dialog visible={!!type} onDismiss={() => setType(null)}><Dialog.Title>{type === 'deposit' ? 'Ajouter des fonds' : 'Effectuer une dépense'}</Dialog.Title><Dialog.ScrollArea><ScrollView style={{ maxHeight: Math.max(80, height * 0.45) }} contentContainerStyle={styles.dialogScroll} keyboardShouldPersistTaps="handled"><ValidatedInput label="Désignation" required errorText={designation !== '' && designation.trim().length < 2 ? 'Désignation requise (2 caractères minimum).' : undefined} value={designation} onChangeText={setDesignation} /><WholeNumberInput required label="Montant" value={amount} onChangeText={setAmount} max={MAX_CASH_AMOUNT} left={<TextInput.Icon icon="cash" />} />{wouldGoNegative && <HelperText type="error" visible>{canOverrideNegative ? `Ce montant dépasse la caisse actuelle (${money(balance)}) : elle passera à ${money(balance - wholeOrNaN(amount,{max:MAX_CASH_AMOUNT}))}.` : `Ce montant dépasse la caisse actuelle (${money(balance)}) : la dépense est refusée.`}</HelperText>}{!!mutation.error && <HelperText type="error" visible>{readableError(mutation.error)}</HelperText>}</ScrollView></Dialog.ScrollArea><Dialog.Actions style={{ flexWrap: 'wrap' }}><AppButton mode="text" onPress={() => setType(null)}>Annuler</AppButton><AppButton loading={mutation.isPending} disabled={!valid || mutation.isPending} onPress={() => { if (wouldGoNegative && canOverrideNegative) { setConfirmNegative(true); return; } mutation.mutate({}); }}>{mutation.isPending?'Enregistrement…':'Confirmer'}</AppButton></Dialog.Actions></Dialog><ConfirmDialog visible={confirmNegative} title="Caisse insuffisante" message={`Cette dépense de ${money(wholeOrNaN(amount,{max:MAX_CASH_AMOUNT}))} dépasse la caisse actuelle (${money(balance)}) : elle passera à ${money(balance - wholeOrNaN(amount,{max:MAX_CASH_AMOUNT}))}. Continuer quand même ?`} loading={mutation.isPending} onCancel={() => setConfirmNegative(false)} onConfirm={() => mutation.mutate({ confirmNegative: true })} /><Dialog visible={closureOpen} onDismiss={()=>!closure.isPending&&setClosureOpen(false)}><Dialog.Title>Clôturer la caisse</Dialog.Title><Dialog.ScrollArea><ScrollView style={{ maxHeight: Math.max(80, height * 0.45) }} contentContainerStyle={styles.dialogScroll} keyboardShouldPersistTaps="handled"><Text>Montant attendu : {money(balance)}</Text><WholeNumberInput required label="Montant réellement compté" value={countedAmount} onChangeText={setCountedAmount} max={MAX_CASH_AMOUNT} /><TextInput mode="outlined" label={closureNoteRequired?'Note (obligatoire : écart au-delà du seuil autorisé)':'Note (facultatif)'} accessibilityLabel={closureNoteRequired?'Note (obligatoire : écart au-delà du seuil autorisé)':'Note (facultatif)'} value={closureNote} onChangeText={setClosureNote} multiline/><Text>Écart : {money(closureDifference)}</Text>{closureNoteRequired&&closureNote.trim().length<3&&<HelperText type="error" visible>Cet écart dépasse le seuil autorisé ({money(varianceThreshold)}) : une justification d’au moins 3 caractères est obligatoire.</HelperText>}{!!closure.error&&<HelperText type="error" visible>{readableError(closure.error)}</HelperText>}</ScrollView></Dialog.ScrollArea><Dialog.Actions style={{ flexWrap: 'wrap' }}><AppButton mode="text" disabled={closure.isPending} onPress={()=>setClosureOpen(false)}>Annuler</AppButton><AppButton loading={closure.isPending} disabled={closure.isPending||!(wholeOrNaN(countedAmount,{max:MAX_CASH_AMOUNT})>=0)||(closureNoteRequired&&closureNote.trim().length<3)} onPress={()=>closure.mutate()}>{closure.isPending?'Enregistrement…':'Valider la clôture'}</AppButton></Dialog.Actions></Dialog></Portal>
