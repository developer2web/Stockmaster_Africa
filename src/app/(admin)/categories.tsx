...
import { Card,Dialog,HelperText,Portal,Switch } from 'react-native-paper';
...
export default function Categories(){
  ...
  const{control,handleSubmit,reset,formState:{isDirty,isValid}}=useForm<CategoryInput>({resolver:zodResolver(categorySchema),defaultValues:{name:'',description:'',isActive:true},mode:'onChange'});
  ...
  return <AdminPage title="Catégories" action={<AppButton icon="plus" accessibilityLabel="Ajouter une catégorie" onPress={()=>show()}>Ajouter</AppButton>}>
    ...
    {shown.map(item=><Card key={item.id} mode="outlined" onPress={()=>show(item)}><Card.Title title={item.name} subtitle={item.description||'Sans description'} right={()=><StatusChip style={{marginRight:12}} status={item.is_active?'active':'archived'}/>} /></Card>)}
    ...
    <Portal><Dialog visible={open} dismissable={!isDirty&&!mutation.isPending} onDismiss={()=>!isDirty&&!mutation.isPending&&setOpen(false)}><Dialog.Title>{editing?'Modifier la catégorie':'Nouvelle catégorie'}</Dialog.Title><Dialog.Content>
    ...
    </Dialog.Content><Dialog.Actions style={{ flexWrap: 'wrap' }}><AppButton mode="text" disabled={mutation.isPending} onPress={()=>setOpen(false)}>Annuler</AppButton><AppButton loading={mutation.isPending} disabled={!isValid||!isDirty||mutation.isPending} onPress={handleSubmit(value=>mutation.mutate(value))}>Enregistrer</AppButton></Dialog.Actions></Dialog></Portal>
  </AdminPage>;
}
