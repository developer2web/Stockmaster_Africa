...
export default function CompleteProfile() {
  ...
  return <AuthScreen title="Finaliser votre espace" subtitle="Étape 2 sur 2 · Ces informations configurent votre boutique."><Card mode="contained" style={{ backgroundColor: '#E1F1F2' }}><Card.Content style={{ gap: 6 }}><Text variant="titleMedium" style={{ fontWeight: '800', color: '#084B50' }}>Dernière étape</Text><Text>Après cela, vous pourrez consulter les abonnements et les conditions d’essai disponibles pour votre compte.</Text></Card.Content></Card><Text variant="labelLarge" style={{ color: '#084B50', fontWeight: '800' }}>Informations obligatoires *</Text><FormField control={control} name="companyName" label="Nom de l’entreprise *" /><FormField control={control} name="storeName" label="Nom de la première boutique *" /><Controller control={control} name="countryCode" render={({field})=><SelectField label="Pays d’activité *" value={field.value} onChange={(value)=>field.onChange(value??'GN')} options={supportedCountries.map((country)=>({label:`${country.name} — ${country.currency}`,value:country.code}))}/>} />{!!error && <HelperText type="error" visible>{error}</HelperText>}<AppButton onPress={submit} loading={formState.isSubmitting}>Créer mon espace et continuer</AppButton><AppBackButton fallback="/" /></AuthScreen>;
}
