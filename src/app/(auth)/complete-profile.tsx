...
export default function CompleteProfile() {
  ...
  return <AuthScreen title="Finaliser votre espace" subtitle="Étape 2 sur 2 · Ces informations configurent votre boutique."><Card mode="contained" style={{ backgroundColor: theme.colors.primaryContainer }}><Card.Content style={{ gap: 6 }}><Text variant="titleMedium" style={{ fontWeight: '800', color: theme.colors.onPrimaryContainer }}>Dernière étape</Text><Text style={{ color: theme.colors.onPrimaryContainer }}>Renseignez les champs marqués *. Après cela, vous pourrez consulter les abonnements et les conditions d’essai disponibles pour votre compte.</Text></Card.Content></Card><FormField control={control} name="companyName" label="Nom de l’entreprise" required /><FormField control={control} name="storeName" label="Nom de la première boutique" required /><Controller control={control} name="countryCode" render={({field})=><SelectField label="Pays d’activité" required value={field.value} onChange={(value)=>field.onChange(value??'GN')} options={supportedCountries.map((country)=>({label:`${country.name} — ${country.currency}`,value:country.code}))}/>} />{!!error && <HelperText type="error" visible>{error}</HelperText>}<AppButton onPress={submit} loading={formState.isSubmitting}>Créer mon espace et continuer</AppButton><AppBackButton fallback="/" /></AuthScreen>;
}
