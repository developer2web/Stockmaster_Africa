...
export interface Product { id:string; company_id:string; store_id:string; supplier_id:string|null; name:string; description:string|null; sku:string; qr_code:string; barcode:string|null; unit:ProductUnit; purchase_price:number; sale_price:number; low_stock_threshold:number; image_url:string|null; image_urls:string[]; is_active:boolean; created_at:string; bulk_unit_label:string|null; bulk_quantity:number|null; bulk_price:number|null; supplier?:{name:string}|null; product_variants?:ProductVariant[] }
export interface StockLevel { id:string; company_id:string; store_id:string; product_id:string; product_variant_id:string|null; quantity:number; updated_at:string; store?:{name:string}|null; product?:{name:string;sku:string;purchase_price?:number;sale_price?:number;is_active:boolean;low_stock_threshold?:number}|null; variant?:{name:string;sku:string}|null }
...
export interface SaleItem { id:string; sale_id:string; product_id:string; product_variant_id:string|null; purchase_price_snapshot:number; sale_price:number; quantity:number; discount:number; tax_rate_snapshot:number; tax_amount:number; gross_profit:number; line_total:number; product?:{name:string;sku:string;unit?:string|null;has_homonym?:boolean}|null; variant?:{name:string;sku:string}|null }
export interface SaleStockItem { stockLevelId:string; productId:string; variantId:string|null; unit:ProductUnit; name:string; sku:string; lookupCodes:string[]; salePrice:number; purchasePrice:number; available:number; imageUrl:string|null; bulkUnitLabel:string|null; bulkQuantity:number|null; bulkPrice:number|null }
...
export interface ReportFilters { stores:ReportFilterOption[]; employees:ReportFilterOption[]; products:ReportFilterOption[] }
